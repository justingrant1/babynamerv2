...
export default async function MaleOriginPage({ params }: PageProps) {
  const { origin } = await params
  const gender = 'male'
  ...
  // Fetch male names with this origin
  ...
  const structuredData = generateCompleteStructuredData({
    breadcrumbs: [
      ...
      { name: `${originLabel} Names`, url: `/names/male/origin/${origin}` },
    ],
    webPage: {
      title: `Male ${originLabel} Baby Names`,
      description: `Discover strong male ${originLabel.toLowerCase()} baby names. ${ORIGIN_DESCRIPTIONS[origin]}`,
      url: `/names/male/origin/${origin}`,
    },
    nameList: nameData.length > 0 ? {
      ...
      listName: `Male ${originLabel} Baby Names`,
      listDescription: `Strong male ${originLabel.toLowerCase()} names for your baby boy`,
    } : undefined,
    faq: faqs,
    collectionPage: {
      title: `Male ${originLabel} Baby Names`,
      description: `Browse male ${originLabel.toLowerCase()} baby names`,
      url: `/names/male/origin/${origin}`,
